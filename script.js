...
// ─── Lodgify Booking Modal Intercept ─────────────────────────────────────────
// Override window.open BEFORE renderBookNowBox.js (defer) executes.
// When Lodgify calls window.open() with a checkout.lodgify.com URL,
// we capture that URL and show it inside the modal iframe instead.
// script.js loads as a regular (non-deferred) body script, so this IIFE runs
// before the deferred renderBookNowBox.js script fires, guaranteeing the
// patched window.open is already in place when Lodgify tries to open the tab.
(function () {
    var _nativeOpen = window.open;

    function openLodgifyModal(url) {
        var modal = document.getElementById('lodgify-booking-modal');
        var iframe = document.getElementById('lodgify-booking-iframe');
        if (!modal || !iframe) {
            // Fallback: open in new tab if modal DOM not found
            _nativeOpen(url, '_blank');
            return;
        }
        iframe.src = url;
        modal.style.display = 'block';
        // Use rAF so display:block is painted before adding class (enables CSS transition)
    ...
    function closeLodgifyModal() {
        var modal = document.getElementById('lodgify-booking-modal');
        var iframe = document.getElementById('lodgify-booking-iframe');
        if (!modal) return;
        modal.classList.remove('active');
        document.body.style.overflow = '';
        // Clear iframe src after fade-out so no background network activity
        setTimeout(function () {
    ...
    // Override window.open to catch Lodgify checkout redirects
    window.open = function (url, target, features) {
        if (url && typeof url === 'string' && url.indexOf('checkout.lodgify.com') !== -1) {
            openLodgifyModal(url);
            // Return a minimal fake window object so Lodgify's code does not throw
            return { closed: false, focus: function () {}, close: function () {} };
        }
        return _nativeOpen.call(window, url, target, features);
    };

    // Also intercept <a> tag clicks targeting checkout.lodgify.com
    // (covers any fallback anchor Lodgify might render)
    document.addEventListener('click', function (e) {
        var el = e.target;
        // Walk up the DOM to find an anchor in case the click was on a child element
        while (el && el !== document) {
            if (el.tagName === 'A' && el.href && el.href.indexOf('checkout.lodgify.com') !== -1) {
                e.preventDefault();
                e.stopPropagation();
                openLodgifyModal(el.href);
                return;
            }
            el = el.parentNode;
        }
    ...
    // Wire up close button, overlay click, and Escape key
    document.addEventListener('DOMContentLoaded', function () {
        var closeBtn = document.getElementById('lodgify-modal-close');
        var modal = document.getElementById('lodgify-booking-modal');
        var overlay = modal && modal.querySelector('.lodgify-modal-overlay');

        if (closeBtn) {
            closeBtn.addEventListener('click', closeLodgifyModal);
        }
        if (overlay) {
            overlay.addEventListener('click', closeLodgifyModal);
        }
        document.addEventListener('keydown', function (e) {
            if (e.key === 'Escape') {
                var m = document.getElementById('lodgify-booking-modal');
